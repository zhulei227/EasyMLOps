from easymlops.table.regression import RidgeCVRegression
import itertools
from .core import *
...
class VarRuler(TablePipeObjectBase):
    ...
    def __init__(self, y: series_type = None, cols=None,
                 ...
                 use_all_train_data=False, val_split_size=0.1, val_split_type="last", val_shuffle=False, n_bins=10,
                 filter_iv=0.01, filter_psi=0.1, filter_diff=0.1, filter_ratio=0.01, filter_mean=0,
                 filter_udf_func=None,
                 output_types=[("average", "avg"), ("weighted_average", "wavg"), ("linear_regression", "lr")],
                 **kwargs):
        """
        :param y:
        ...
        :param use_all_train_data:是否使用全部的训练数据训练模型
        ...
        :param output_types:输出方式,average平均,weighted_average加权平均,linear_regression回归聚合,以及后缀
        :param kwargs:
        """
        ...
        # 验证集切割方式
        self.use_all_train_data = use_all_train_data
        ...
        self.output_linear_regression = None
    ...
    def udf_fit(self, s, **kwargs):
        ...
        if not self.use_all_train_data:
        ...
        for ruler_feature_num, keep_ruler_rate in self.keep_ruler_rate.items():
            ...
            for _, input_cols in enu_combinations:
                if np.random.random() <= keep_ruler_rate:
                    ...
                    # 2.记录模型
                    self.ruler_totals.append([self.ruler_prefix + f"_{all_idx}", input_cols, model])
                    # 3.预测
                    ...
                    record_perfer_total.append([self.ruler_prefix + f"_{all_idx}", "|".join(input_cols),
                    ...
                    # 补充计算自定义指标：TODO
                    trn_cnt_data["ruler_name"] = self.ruler_prefix + f"_{all_idx}"
                    val_cnt_data["ruler_name"] = self.ruler_prefix + f"_{all_idx}"
        ...
        # 根据output_type
        if "linear_regression" in [item_[0] for item_ in self.output_types]:
            self.output_linear_regression = RidgeCVRegression(self.y)
            self.output_linear_regression.fit(self.udf_transform_expand(s))
        return self

    def udf_transform_expand(self, s, **kwargs):
        values = {}
        for ruler_name, leaf_info in self.used_rulers.items():
            for leaf_index in leaf_info.keys():
                values[f"{ruler_name}_{leaf_index}"] = np.zeros(len(s))

        if kwargs.get("show_process", False):
            print("var ruler transform expand:")
            indexes = tqdm(range(len(self.ruler_totals)))
        else:
            indexes = range(len(self.ruler_totals))
        for index_ in indexes:
            ruler_name, input_cols, model = self.ruler_totals[index_]
            if ruler_name in self.used_rulers:
                leaf_indexes = model.transform(s[input_cols])["ruler_leaf_index"].values
                for i, leaf_index in enumerate(leaf_indexes):
                    if f"{ruler_name}_{leaf_index}" in values:
                        values[f"{ruler_name}_{leaf_index}"][i] = 1.0
        return pd.DataFrame(values)

    def udf_transform(self, s, **kwargs):
        values = {}
        for output_type, output_col in self.output_types:
            if output_type == "linear_regression":
                values[f"{self.ruler_prefix}_{output_col}"] = self.output_linear_regression \
                    .transform(self.udf_transform_expand(s, **kwargs))["pred"].values
            elif output_type in ["average", "weighted_average"]:
                value = np.zeros(len(s))
                if kwargs.get("show_process", False):
                    print("var ruler transform:")
                    indexes = tqdm(range(len(self.ruler_totals)))
                else:
                    indexes = range(len(self.ruler_totals))
                for index_ in indexes:
                    ...
                    if ruler_name in self.used_rulers:
                        if output_type == "average":
                            value = value + model.transform(s[input_cols])["ruler_leaf_index"] \
                ...
                value = value / (len(self.used_rulers) + 1e-3)
                values[f"{self.ruler_prefix}_{output_col}"] = value
            else:
                raise Exception("output_type should be average or weighted_average or linear_regression")
    ...
    def udf_transform_single(self, s: dict_type, **kwargs):
        ...
        for output_type, output_col in self.output_types:
            if output_type == "linear_regression":
                values[f"{self.ruler_prefix}_{output_col}"] = self.output_linear_regression \
                    .transform(self.udf_transform_expand(pd.DataFrame(s, index=[0])))["pred"].values[0]
            elif output_type in ["average", "weighted_average"]:
                value = 0.0
                for ruler_name, input_cols, model in self.ruler_totals:
                    if ruler_name in self.used_rulers:
                        ...
                        if output_type == "average":
                            ...
                            value += self.used_rulers[ruler_name].get(
                                int(model.transform_single(s_)["ruler_leaf_index"]), 0.0)
                value = value / (len(self.used_rulers) + 1e-3)
                values[f"{self.ruler_prefix}_{output_col}"] = value
            else:
                raise Exception("output_type should be average or weighted_average or linear_regression")
    ...
    def udf_get_params(self) -> dict_type:
        ...
        if "linear_regression" in [item_[0] for item_ in self.output_types]:
            output_linear_regression_params = self.output_linear_regression.get_params()
        else:
            output_linear_regression_params = None
        params = {"cols": self.cols,
                  ...
                  "native_fit_params": self.native_fit_params,
                  "use_faster_predictor": self.use_faster_predictor,
                  "use_all_train_data": self.use_all_train_data,
                  ...
                  "output_linear_regression_params": output_linear_regression_params}
        return params
    ...
    def udf_set_params(self, params: dict):
        ...
        self.use_all_train_data = params["use_all_train_data"]
        ...
        self.output_linear_regression = None
        if "linear_regression" in [item[0] for item in self.output_types]:
            self.output_linear_regression = RidgeCVRegression()
            self.output_linear_regression.set_params(params["output_linear_regression_params"])
