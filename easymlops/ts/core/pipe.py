...
class PipeBase(PipeObjectBase):
    ...
    def get_params(self) -> dict_type:
        ...
        while issubclass(current_class, PipeBase):
    ...
    def get_match_values_index(x: series_type, values):
        ...
        try:
            index = x_.map(index_map).fillna(False)  # python 3.7,3.8
        except:
            index = x_.apply(lambda x: index_map.get(x, False))  # python 3.10
...
class M2MRollingPipeBase(PipeBase):
    """
    more input More output
    """

    def __init__(self, output_cols: list = None, input_cols: list = None, min_cache_length=None, **kwargs):
    ...
    def udf_transform(self, s: dataframe_type, **kwargs) -> dataframe_type:
        ...
        values = rolling_apply(self.apply_rolling_func, self.min_cache_length,
                               *inputs, n_jobs=1)
        for idx in range(len(self.output_cols)):
            if values.ndim == 1:
                value = values
            else:
                value = values[:, idx]
    ...
    def udf_transform_single(self, s: dict_type, **kwargs) -> dict_type:
        ...
        inputs = []
        for input_col_name in self.input_cols:
            inputs.append(1.0 * np.asarray(self.history_data[input_col_name]))
        inputs = tuple(inputs)
        if len(inputs[0]) == self.min_cache_length:
            values = rolling_apply(self.apply_rolling_func, self.min_cache_length,
                                   *inputs, n_jobs=1)
            for idx in range(len(self.output_cols)):
                if values.ndim == 1:
                ...
                value = np.where(np.isnan(value), .0, value)
                value = np.where(np.isinf(value), .0, value)
                s[self.output_cols[idx]] = value[-1]
            # 最后pop第一个数:没用了
            for input_col_name in self.input_cols:
                self.history_data[input_col_name].pop(0)
        else:
    ...
    def udf_get_params(self) -> dict_type:
        params = {"output_cols": self.output_cols,
                  "input_cols": self.input_cols,
                  "min_cache_length": self.min_cache_length}
        return params

    def udf_set_params(self, params: dict):
        self.output_cols = params["output_cols"]
        self.input_cols = params["input_cols"]
        self.min_cache_length = params["min_cache_length"]

    def reset_cache_data(self):
        self.history_data = {}
        for input_col in self.input_cols:
            self.history_data[input_col] = []
